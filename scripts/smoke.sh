...
DATA_DIR="$DIR" PORT="$PORT" SERVER=fast "$BIN" >/tmp/shrt-smoke.log 2>&1 &
SRV=$!
...
curl -s -o /dev/null -w "%{http_code} allow-origin: %{header_json} " -X OPTIONS "$BASE/api/shorten" 2>/dev/null \
